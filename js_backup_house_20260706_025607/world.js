...
GAME.World = (function () {
  ...
  function updateDayNight(dt) {
    ...
    if (isNightNow) {
      ...
      dark = Math.min(1, Math.sin(Math.min(into, 1) * Math.PI) * 1.4 + 0.35);
    } else {
    ...
    if (sun && hemi) {
      ...
      sun.intensity = 1.1 * (1 - dark) + 0.05;
      hemi.intensity = 0.7 * (1 - dark * 0.85);
  ...
  function updateEnemies(dt, playerPos) {
    const ec = GAME.Config.enemies;
    const st = GAME.State.currentStage && GAME.State.currentStage();
    // Zombies roam AT NIGHT only (and only while the stage allows combat).
    const combat = !!(st && st.spawnEnemies) && !GAME.State.won && isNightNow;
    ...
    for (const e of enemies) {
      ...
      if (d > ec.touchRange) {
        ...
        if (e.lastHit <= 0) {
          ...
          GAME.State.damage(es.damage);
          GAME.Events.emit('enemy:hit', { damage: es.damage });
        }
      }
    }
  }
