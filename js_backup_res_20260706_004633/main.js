...
GAME.UI = (function () {
  ...
  function showGameOver() {
    ...
    el.innerHTML =
      `<div class="overlay-card">` +
      `<h2>\u2620\ufe0f Game Over</h2>` +
      `<p style="color:var(--muted);margin-bottom:16px">Your health reached zero. The quest ends here.</p>` +
...
(function () {
  ...
  function bindHudEvents() {
    ...
    // Death → game over.
    GAME.Events.on('player:death', () => GAME.UI.showGameOver());
  ...
  function loop(now) {
    ...
    // Live HUD: shield + coordinates
    const maxShield = GAME.State.character?.maxShield || GAME.Config.progression.maxShield;
    document.getElementById('bar-sta').style.width =
      Math.min(100, GAME.State.stats.shield / maxShield * 100) + '%';
