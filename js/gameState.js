...
GAME.State = {
  ...
  // ---- Weapons ----
  weapon: 'axe',          // currently equipped weapon id (axe is a starter)
  unlockedWeapons: ['dagger', 'axe'],   // two starters owned from the start
  ...
  damage(amount) {
    ...
    GAME.Events.emit('player:damage', { amount, hp: s.hp });
    if (s.hp === 0) {
      this.deathReason = 'alien';
      GAME.Events.emit('player:death', { reason: 'alien' });
    }
  },
  ...
  eat(itemId) {
    ...
    // Cooking heal: eating food while NEAR A CAMPFIRE restores health —
    // 5% of max HP per food (config: survival.campfireHealFraction), until
    // fully healed. Away from a campfire, food only restores hunger (no heal).
    const maxHP = (this.character && this.character.maxHP) || GAME.Config.progression.maxHP;
    const pos = GAME.Player && GAME.Player.getPosition && GAME.Player.getPosition();
    const nearFire = pos && GAME.World && GAME.World.nearCampfire && GAME.World.nearCampfire(pos.x, pos.z);
    if (nearFire) {
      const healFrac = (typeof sv.campfireHealFraction === 'number') ? sv.campfireHealFraction : 0.05;
      const hpBefore = this.stats.hp;
      this.stats.hp = Math.min(maxHP, this.stats.hp + maxHP * healFrac);
      if (this.stats.hp > hpBefore) {
        GAME.Events.emit('player:heal', { amount: this.stats.hp - hpBefore, hp: this.stats.hp, cooked: true });
      }
    }
  ...
  survivalTick(dt, pos, moving) {
    ...
    const sv = GAME.Config.survival;
    const s = this.stats;
    const drain = sv.maxHunger / (sv.oxygenDurationSec || 1800);
    s.hunger = Math.max(0, s.hunger - drain * dt);
    if (s.hunger <= 0 && s.hp > 0) {
      s.hp = 0;
      this.deathReason = 'oxygen';
      GAME.Events.emit('player:death', { reason: 'oxygen' });
    }
    GAME.Events.emit('survival:update', { hunger: s.hunger, hp: s.hp, oxygen: s.hunger });
  },
  ...
  itemDef(id) {
    return (GAME.Config.resources && GAME.Config.resources[id])
        ...
        || (GAME.Config.weapons && GAME.Config.weapons[id])
        || null;
  },

  hasShipParts() {
    const parts = GAME.Config.story?.shipParts || ['engine', 'wing', 'spare_metal'];
    return parts.every((p) => this.getResourceCount(p) >= 1);
  },
  ...
  isRecipeUnlocked(recipeId) { return !!this.getRecipe(recipeId); },

  canCraft(recipeId) {
    const r = this.getRecipe(recipeId);
    if (!r) return false;
    if (!this.isRecipeUnlocked(recipeId)) return false;
    // Weapon recipes can't be crafted twice — you already own it.
    if (r.weapon && this.unlockedWeapons.includes(r.output)) return false;
  ...
  craft(recipeId) {
    ...
    if (r.weapon) {
      // Weapon recipe: unlock the weapon and equip it (grant its XP once).
      if (!this.unlockedWeapons.includes(r.output)) this.unlockedWeapons.push(r.output);
      const wdef = GAME.Config.weapons[r.output];
      if (wdef && wdef.xp) this.addXP(wdef.xp);
      this.equipWeapon(r.output);
      GAME.Events.emit('craft:success', { recipeId, output: r.output, amount: 1, weapon: true });
      return true;
    }
  ...
  load() {
    try {
      ...
      if (data.stats) {
        ...
        // Migrate old saves: the STA bar became a SHIELD. If a save predates
        // the rename, seed the shield from any legacy stamina value (or full).
        if (typeof this.stats.shield !== 'number') {
          this.stats.shield = (typeof this.stats.stamina === 'number')
            ? this.stats.stamina
            : ((this.character && this.character.maxShield) || GAME.Config.progression.maxShield);
        }
        delete this.stats.stamina;
      }
      if (data.inventory) this.inventory = data.inventory;
      if (data.hotbar) this.hotbar = data.hotbar;
      while (this.hotbar.length < GAME.Config.hotbarSlots) this.hotbar.push(null);
      if (this.hotbar.length > GAME.Config.hotbarSlots) this.hotbar.length = GAME.Config.hotbarSlots;
