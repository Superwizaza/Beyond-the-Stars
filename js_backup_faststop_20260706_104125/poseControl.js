...
GAME.PoseControl = (function () {
  ...
  const cfg = {
    ...
    cadenceWalkAmp: 0.15,      // min peak-to-peak knee bob (body-relative) = walking
    ...
    cadenceReleaseAmp: 0.10,  // once sprinting, keep going until bob drops below this
  ...
  function computeCadence(pose, t) {
    ...
    for (const nm of ['leftKnee','rightKnee']) {
      const k = kp(pose, nm);
      if (k && k.score >= 0.20) { kneeSum += (hipAnchor !== null ? (k.position.y - hipAnchor) : k.position.y) / REF; kneeN++; }
    }
    if (kneeN === 0) {
    ...
    if (kneeVal !== null) { _kneeEMA = (_kneeEMA === null) ? kneeVal : (0.65 * _kneeEMA + 0.35 * kneeVal); kneeVal = _kneeEMA; }
