...
GAME.World = (function () {
  ...
  function addRocks() {
    ...
    for (let i = 0; i < cfg.rockCount; i++) {
      ...
      rock.position.set(p.x, groundHeight(p.x, p.z) + s * 0.5, p.z);
  ...
  function updateEnemies(dt, playerPos) {
    ...
    for (const e of enemies) {
      ...
      if (d > ec.touchRange) {
        e.x += (dx / d) * ec.speed * dt;
        e.z += (dz / d) * ec.speed * dt;
  ...
  function placeBlock(itemId, x, z) {
    const def = GAME.Config.craftables[itemId];
    if (!def || !def.placeable) return null;
    const size = GAME.Config.build.blockSize;
    const gy = groundHeight(x, z);
    const isLight = (itemId === 'campfire' || itemId === 'torch');

    const mat = new THREE.MeshStandardMaterial({
      color: def.color || 0x999999,
      emissive: isLight ? def.color : 0x000000,
    ...
    let light = null;
    if (isLight) {
      light = new THREE.PointLight(def.color || 0xffaa33, 1.2, 18);
