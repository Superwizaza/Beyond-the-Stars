...
GAME.PoseControl = (function () {
  ...
  function computeCadence(pose, t) {
    ...
    let legInst = energyOf(['leftKnee','rightKnee','leftAnkle','rightAnkle','leftHip','rightHip'], 0.25);
    ...
    // Smooth both signals so a single jittery frame can't flip states.
    ...
    // Classify:
    //  • RUN  = legs at least walking AND arms pumping, or combined very high.
    //  • WALK = legs moving (arms mostly straight/at sides — low arm energy).
    //  • IDLE = below the walk threshold (small fidget tolerated).
    const legsMoving = _cadEnergy >= cfg.cadenceWalkThresh;
    const armsPumping = _cadArm >= cfg.cadenceArmPumpThresh;
    if ((legsMoving && armsPumping) || combined >= cfg.cadenceRunThresh) return 'run';
    if (legsMoving) return 'walk';
    return 'idle';
  }
  // ============================================================
