...
GAME.PoseControl = (function () {
  ...
  const cfg = {
    ...
    turnGain: 9,            // steer pixels per unit smoothed slide velocity
    turnVelSmooth: 0.7,     // EMA on hand velocity
  ...
  function computeTurn(pose, t) {
    ...
    let steer = clamp(_turnVel[best] * cfg.turnGain, -1, 1); // - = left, + = right
    steer = turnFilter(steer, t);                            // One-Euro smoothing
    if (Math.abs(steer) < cfg.turnDeadzone) return 0;
    return steer * cfg.maxTurnPixels;                        // → synthetic movementX
  }
