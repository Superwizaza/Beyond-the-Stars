...
GAME.PoseControl = (function () {
  ...
  const cfg = {
    ...
    cadenceWalkAmp: 0.06,      // min peak-to-peak knee bob (body-relative) = walking
  ...
  function computeCadence(pose, t) {
    ...
    // Classify from step AMPLITUDE + FREQUENCY (+ torso gate + arm pump):
    const stepping   = amp >= cfg.cadenceWalkAmp;         // lifting knees rhythmically
    const torsoMoving = _cadTorso >= cfg.cadenceTorsoThresh;
    const moving = stepping && torsoMoving;
    const fast = steps >= cfg.cadenceRunFreq;             // high step cadence
    const armsPumping = _cadArm >= cfg.cadenceArmPumpThresh;
    if (moving && (fast || armsPumping)) return 'run';
