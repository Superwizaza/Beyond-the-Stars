...
GAME.PoseControl = (function () {
  ...
  const cfg = {
    ...
    cadenceWinSec: 0.9,        // rolling window length (seconds)
    cadenceWalkAmp: 0.10,      // min peak-to-peak knee bob (body-relative) = walking
    ...
    swingRevolution: 5.6,     // radians of continuous same-direction travel to
                              // count as a full circle (~2π; slightly under so a
                              // near-complete loop still fires)
    swingWindow: 1.4,         // must complete the revolution within this many sec
  ...
  function computeCadence(pose, t) {
    ...
    // Fall back to ankles, then hips, if knees aren't visible.
    if (kneeN === 0) {
      for (const nm of ['leftAnkle','rightAnkle']) {
        const k = kp(pose, nm);
        if (k && k.score >= 0.20) { kneeSum += (hipY !== null ? (k.position.y - hipY) : k.position.y) / REF; kneeN++; }
      }
    }
    ...
    // TORSO must move WITH the legs (seated leg-swings stay idle).
    const torsoInst = energyOf(['leftShoulder','rightShoulder','leftHip','rightHip'], 0.25);
    const aDn = (typeof cfg.cadenceDecay === 'number') ? cfg.cadenceDecay : cfg.cadenceSmoothing;
    const emaT = (torsoInst >= _cadTorso ? cfg.cadenceSmoothing : aDn);
    _cadTorso = emaT * _cadTorso + (1 - emaT) * torsoInst;

    // ARMS pumping → run signal.
    const armInst = energyOf(['leftWrist','rightWrist','leftElbow','rightElbow'], 0.25);
    const emaA = (armInst >= _cadArm ? cfg.cadenceSmoothing : aDn);
    _cadArm = emaA * _cadArm + (1 - emaA) * armInst;

    // SIMPLIFIED: no walk/run split. SPRINT whenever the legs are GENUINELY
    // moving (a real march/knee-lift, not a small shuffle) together with the
    // torso. Arms are ignored entirely. Otherwise idle.
    const stepping    = amp >= cfg.cadenceWalkAmp;        // genuine rhythmic knee lift
    const torsoMoving = _cadTorso >= cfg.cadenceTorsoThresh;
    if (stepping && torsoMoving) return 'run';            // legs moving → sprint
    return 'idle';
  }
  // ============================================================
