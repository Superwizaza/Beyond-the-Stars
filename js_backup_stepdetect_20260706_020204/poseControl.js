...
GAME.PoseControl = (function () {
  ...
  function computeCadence(pose, t) {
    ...
    let legInst = liftEnergyOf(['leftKnee','rightKnee','leftAnkle','rightAnkle'], 0.22);
    ...
    // Smooth signals so a single jittery frame can't flip states. ASYMMETRIC:
    // while motion is RISING use cadenceSmoothing (stable detection); while it's
    // FALLING use the lower cadenceDecay so the signal drops fast and the
    // character returns to idle quickly after you stop.
    const aUp = cfg.cadenceSmoothing;
    const aDn = (typeof cfg.cadenceDecay === 'number') ? cfg.cadenceDecay : cfg.cadenceSmoothing;
    const ema = (prev, inst) => { const a = (inst >= prev ? aUp : aDn); return a * prev + (1 - a) * inst; };
    _cadEnergy = ema(_cadEnergy, legInst);                           // legs
    _cadTorso  = ema(_cadTorso, torsoInst);                          // torso
    _cadArm    = ema(_cadArm, armInst);                              // arms
