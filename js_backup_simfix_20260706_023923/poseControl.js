...
GAME.PoseControl = (function () {
  ...
  const cfg = {
    ...
    maxTurnPixels: 9,       // synthetic movementX at full turn, per frame
                            // (lowered for controllable, proportional turning)
    turnDeadzone: 0.14,     // solid deadzone: a hand near center (incl. a
                            // pumping arm) won't steer — only a deliberate slide does
    ...
    cadenceReleaseAmp: 0.055,  // once sprinting, keep going until bob drops below this
    cadenceCoastSec: 0.45,     // keep sprinting this long through brief between-step dips
    // ===============================================================
  ...
  function computeCadence(pose, t) {
    ...
    // ABSOLUTE knee vertical (body-scaled) — NOT relative to the hip. Measuring
    // knee-minus-hip let noisy hip keypoints fake a step when only the torso
    // moved. Absolute knee bob only changes when you actually lift your knees,
    // so torso-only movement no longer triggers sprint.
    let kneeSum = 0, kneeN = 0;
    for (const nm of ['leftKnee','rightKnee']) {
      const k = kp(pose, nm);
      if (k && k.score >= 0.20) { kneeSum += k.position.y / REF; kneeN++; }
    }
    if (kneeN === 0) {
